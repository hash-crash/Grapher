 ...
 * 
 */
function drawFlipIndication(e1, e2) {
    ...
    let flips = possibleFlipsPoints(p1, p2, p3, p4, edgesWithoutEither);
...
function handleClickMatchingsMode(mousePos) {
    ...
    if (selectedEdge === -1) {
        ...
        if (selectedVx !== -1) {
            ...
            selectedVx = -1;
            flipEdges = edgesWithPossibleFlipsM(selectedEdge);
 ...
 */
function edgesWithPossibleFlipsM(e) {
...
function isFlipPossibleMatching(e1, e2) {
    return possibleFlips(e1, e2) !== null;
}
...
function possibleFlips(e1, e2) {
    ...
    return possibleFlipsPoints(p1, p2, p3, p4, edgesWithoutEither);
}


/**
 * 
 * @param {[Number, Number]} p1 coords
 * @param {[Number, Number]} p2 coords
 * @param {[Number, Number]} p3 coords
 * @param {[Number, Number]} p4 coords
 * @param {[[Number, Number]]} otherEdges edges list, same as wg.state.edges, but without the edge connecting p1-p2 and p3-p4 
 * 
 * @see possibleFlips
 */
function possibleFlipsPoints(p1, p2, p3, p4, otherEdges) {
